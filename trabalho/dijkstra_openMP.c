...
int *dijkstra(struct Graph *graph, int source) {
	...
	#pragma omp parallel private(v, k)
    {
		...
		for (v = 1; v < nNodes; v++) {
			...
			// Espera todas as threads e encerra caso nada tenha sido encontrado (desconexo)
			#pragma omp barrier
			if (min == -1) {
				break;
			}
			...
			// Atualiza o grafo com o menor caminho
			#pragma omp for
